...
month.func = function(month){     
  ...
  for (i in 1:length(month)){
    month[i] = month_name[as.numeric(month[i])]
  }
  return(month)                 
}                                                               
...
TRAIN.CLEAN.3$Day = rep(c("Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday",
                          "Monday"),length.out=1461)
TEST.CLEAN.3$Day  = rep(c("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday",
                          "Saturday"),length.out=114)
...
cels.2.fahr = function(tempreature){
  for (i in 1:length(tempreature)){
    tempreature[i] = tempreature[i]*1.8 + 32
  }
  return(tempreature)
}
...
SD.TEMP.TRAIN.pre = TRAIN.FINAL %>%
                    group_by(Day, Month)%>%
                    summarise(SD.TEMPERATURE = sd(Temperature)) %>%
                    ungroup()

for(x in 1:12){
  for(y in 1:7){
    for(i in 1:length(SD.TEMP.TRAIN.pre$SD.TEMPERATURE)){
      if(SD.TEMP.TRAIN.pre$Month[i] == rownames(SD.TEMP.TRAIN)[x])
        if(SD.TEMP.TRAIN.pre$Day[i] == colnames(SD.TEMP.TRAIN)[y])
          SD.TEMP.TRAIN[x,y] = SD.TEMP.TRAIN.pre$SD.TEMPERATURE[i]
    }
  }
}
...
CV.MONTH.TRAIN = SD.TEMP.TRAIN %>%
                 apply(1,CV.func)
                 

CV.DAY.TRAIN = SD.TEMP.TRAIN %>%
                 apply(2,CV.func) 
...
for (i in 1:length(TRAIN.FINAL$Day)){
  if((TRAIN.FINAL$Day[i] =="Monday")|
     (TRAIN.FINAL$Day[i]=="Tuesday")|
     (TRAIN.FINAL$Day[i]=="Wednesday")|
     (TRAIN.FINAL$Day[i]=="Thursday")|
     (TRAIN.FINAL$Day[i]=="Friday")){
    WEEKDAY.PRESSURE = c(WEEKDAY.PRESSURE, TRAIN.FINAL$Pressure[i])
  }
  else{
...
Answer (4 Points): 
First, we get the t-score by the test. A small t-score indicates that the groups are similar. Since our t-score is 0.64391, we can suggest that there is not much difference between the pressure on weekdays and weekends. By reading the p-value since the p-value of 0.5198 is not smaller than 0.05, we fail to reject the null hypothesis, which suggests that the actual difference in means of pressure on weekdays and weekends is 0. Also, the 95 percent confidence interval shows that the mean difference is between 8.83885 and 17.473970. Besides, by the direct calculus, the difference between x and y is 4.328.

From all of the information above, we can conclude that there is no apparent difference between the pressure on weekdays and weekends.
...
for(i in 2: length(TRAIN.FINAL$`Pressure Percent Change`)){
  TRAIN.FINAL$`Pressure Percent Change`[i] = 
        100*((TRAIN.FINAL$Pressure[i] - TRAIN.FINAL$Pressure[i-1])/TRAIN.FINAL$Pressure[i-1])
}
...
for(i in 2: length(TRAIN.FINAL$`Pressure 3-Day Average`)){
  TRAIN.FINAL$`Pressure 3-Day Average`[i] = 
        (TRAIN.FINAL$Pressure[i-1] + TRAIN.FINAL$Pressure[i] + TRAIN.FINAL$Pressure[i+1])/3
}
