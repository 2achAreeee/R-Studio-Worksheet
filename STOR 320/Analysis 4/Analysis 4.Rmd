...
for(row in 1:nrow(OUT.MAE)) {
  for(col in 1:ncol(OUT.MAE)) {
    DATA3=DATA2%>%
          ...
          select(predict) %>%
          unnest()
    OUT.MAE[row,col]=MAE.func(actual=DATA3$W,predict=DATA3$.fitted);
  }
}
...
Answer *(2 Points)*: According to the plot, we find out that the top five models approximately predict similar maximum water temperatures values for the data since all the lines from the top five models are covered by each other so we cannot distinguish them. Thus, I would conclude that the top five models have approximately the same predicted values.
...
Answer: Based on these figures, I will suggest using (3,4,5) as $I$ and using (6,7,8) as $J$ since these points correspond with the lowest avgMAE on their respective plots. These two figures tell us that it is not as we predicted that having more data on maximum air temperature and more Julian days in the prediction model would benefit the accuracy of our prediction.
...
BEST.func=function(data){
  output = data[which.min(data$MAE), 1:2]
  as.numeric(output)
}
...
```{r,eval=T}
ggplot(DATA) + 
  geom_point(aes(x=W,y=predict(train.model.func(DATA,2,6))), alpha=0.05,stroke=0)+
...
```{r,eval=T}
ggplot(DATA) + 
  geom_point(aes(x=D,y=residuals(train.model.func(DATA,2,6))), alpha=0.05,stroke=0) +
