...
The purpose of adding 1 for th elog transformed is to avoid log(0) which is undefined.

**Question:** In complete sentences, what is the purpose of adding 1 in the creation of the year variable?

Because we want to count the year it shows as the first year. We need to add one when we try to calculate the time length period.
...
```{r,eval=T}
ggplot(bgg3, mapping = aes(x= num_votes, y= geek_rating)) +
  geom_point()

ggplot(bgg3, mapping = aes(x= owned, y= geek_rating)) +
  geom_point()

ggplot(bgg3, mapping = aes(x= log_vote, y= geek_rating)) +
  geom_point()

ggplot(bgg3, mapping = aes(x= log_own, y= geek_rating)) +
  geom_point()
```

**Question:** In complete sentences, describe how the relationship changes when you take the log of the independent variable.

The relationship change to show as a slowly increase pattern. It shows the affect of geek_rating much obviously, and easy for us to read.
...
bgg4= bgg3 %>%
        mutate(Set=sample(x=c('Train','Test'), size =4999, replace = T, prob = c(0.8,0.2)))
...
```{r,eval=T}
lm1 = lm(formula = geek_rating ~ log(num_votes),data=train.bgg)
lm2 = lm(formula = geek_rating ~ log(owned),data=train.bgg)
lm3 = lm(formula = geek_rating ~ log(owned) + vote.per.year + weight,data=train.bgg)
```
...
```{r,eval=T}
test.bgg2 = test.bgg %>%
            mutate(lm1_pred = predict(lm1, newdata=.),
                      lm2_pred = predict(lm2, newdata=.),
                      lm3_pred = predict(lm3, newdata=.),
                      lm1_resid = .$geek_rating - lm1_pred,
                      lm2_resid = .$geek_rating - lm2_pred,
                      lm3_resid = .$geek_rating - lm3_pred)
...
MAE.func(test.bgg2$lm1_resid)

MAE.func(test.bgg2$lm2_resid)

MAE.func(test.bgg2$lm3_resid)

```

**Question:** Which model does the best job at predicting the geek rating of these board games?

The third model lm3 = $\textrm{geek_rating}=\beta_0+\beta_1 log(\textrm{owned})+ \beta_2 \textrm{vote.per.year}+ \beta_3 \textrm{weight} + \epsilon$ does the best job i all these three model since its MAE output 0.1696334 is the smallest among of them.The smaller MAE would be better because the MAE is a from of error. The result would be the best if MAE close to 0.
...
cv <- crossv_kfold(bgg3, k=10)
models <- map(cv$train, ~lm3)
summary(map2_dbl(models, cv$test, modelr::mae))

mean(map2_dbl(models, cv$test, modelr::mae))



abs(MAE.func(test.bgg2$lm3_resid) - mean(map2_dbl(models, cv$test, modelr::mae))) 
...
The absolute difference between the out-of-sample mean absolute error measured using a test set and the mean absolute error measured using cross-validation is  0.002096.
