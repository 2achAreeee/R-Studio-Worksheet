...
```{r, eval=T}
E100 = apply(matrix(rbinom(N*100,k,p),N),1,est_kp)
E250 = apply(matrix(rbinom(N*250,k,p),N),1,est_kp)
...
When the sample size increase, the bias and the MSE change decrease.
...
```{r, eval=T}
ggplot(
  data = filter(df_k, Estimates <=50, Estimates >= 0), 
