...
```{r, eval=T}
flights %>%
  inner_join(plane_ages, by = NULL) %>%
  group_by(age) %>%
  summarise(avg_dep_delay = mean(arr_delay, na.rm = TRUE), .groups='drop') %>%
...
```
Answer: Those flights got canceled, and have not been assigned any plane to fly the flights.
...
Answer: American Airlines (AA) and Envoy Air (MQ) report fleet numbers rather than tail numbers so their data can’t be matched.